...
async function sendCardImage(client, replyToken, card) {
  ...
  if (!imageUrl) {
    await reply(client, replyToken, { type: 'text', text: `🎴 ${card.id}\n\n${card.body}\n\nCertified by 十元醫師` });
    return;
  }
  ...
  await reply(client, replyToken, messages);
}
...
async function handleDraw(userId, displayName, client, replyToken) {
  ...
  if (alreadyDrew) {
    await reply(client, replyToken, { type: 'text', text: `🎴 今天已經抽過卡了！\n明天再來拆包～\n\n每天報到都有機會抽卡 ✨` });
    return;
  }
...
async function handleMessage(event, client) {
  ...
  if (keywords.checkin.some(k => text.includes(k))) {
    ...
    await reply(client, replyToken, { type: 'text', text: msg });
    ...
    await reply(client, replyToken, { type: 'text', text: handleHelp() });

  } else {
    await reply(client, replyToken, { type: 'text', text: `輸入「help」看全部指令 🙂\n\n或者直接點下方選單！` });
  }
}
...
async function handleFollow(event, client) {
  ...
  await reply(client, event.replyToken, { type: 'text', text: handleWelcome(displayName) });
}
